...
#include "EEPROM.h"
#include "RTCC.h"
#include "pic_server.X/Sensors.h"
#include "pic_server.X/Actuators.h"
...
#endif
{
    ...
    openeeprom(EEPROM_CONTROL, EEPROM_BAUD); //opens eeprom and onfigures it
    RtcInit();
    update_rtcc();

    char api_key1[] = "21aeb8ebc4414323944606e8103ccc21";
    struct PROFILE trial;

    trial.publishingRate=10;
    trial.samplingRate=5;
    strncpy(trial.secretKey,api_key1,32);
    lcd_putchar1("writing eeprom");
    writeProfileToEeprom(&trial,sizeof(trial));

    struct PROFILE config;
    DelayMs(100);

    readProfileFromEeprom(&config,sizeof(config));
    char test[10];
    sprintf(test,"eeprom ""%d",config.samplingRate);
    lcd_putchar2(test);
