...
/* Read one byte from the LCD controller.  rs is the RS signal, 0 selects
	 busy flag (bit 7) and address counter, 1 selects the data register */
...
void hd44780_wait_ready(unsigned int islong);
...
/* Function set.  if8bit selects an 8-bit data path, twoline arranges
 	for a two-line display, font5x10 selects the 5x10 dot font 
	(5x8 dots if clear) */
